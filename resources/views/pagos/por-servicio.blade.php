...
@section('content')
<h2 class="text-3xl font-extrabold text-rose-700 mb-6">Pagos por Servicio</h2>

{{-- Formulario para filtrar y exportar --}}
<form id="filtro-form" method="GET" action="{{ route('pagos.por-servicio') }}" class="mb-8 flex flex-wrap items-end gap-4 text-base">
    <label>
        Desde:
        <input type="date" id="desde" name="desde" value="{{ $desde }}" required class="border rounded px-2 py-1">
    </label>
    <label>
        Hasta:
        <input type="date" id="hasta" name="hasta" value="{{ $hasta }}" required class="border rounded px-2 py-1">
    </label>
    ...
    <table class="w-full text-lg border-collapse">
        <thead>
            <tr class="bg-rose-200 text-gray-800 uppercase tracking-wide">
                <th class="border border-gray-300 px-4 py-3 text-left">🛎️ Servicio</th>
        ...
        <tbody>
            @forelse($resultados as $fila)
                <tr class="hover:bg-rose-50">
                    <td class="border-b border-gray-200 px-4 py-3">{{ $fila->servicio }}</td>
...
function exportarPDF() {
    ...
    if (desde && hasta) {
        const url = `{{ route('pagos.exportar.servicios') }}?desde=${desde}&hasta=${hasta}`;
